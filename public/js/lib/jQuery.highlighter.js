        ...
        var selText;
            ...
            return this.each(function () {
                ...
                function insertSpanAfterSelection(clicks) {
                    ...
                    if (window.getSelection) {
                        ...
                        if (sel.getRangeAt && sel.rangeCount) {
                            ...
                            if (!isIE && $.trim(selText) === $.trim(expandedSelRange.startContainer.innerText)) {
                                ...
                                topOffset += rect.height + 40;  // magic number :>
                                var charsOffset; 
                                position = $(dummy).offset();
                                /**/

                                /*selText.length >= 90 ? position.left = 263 : position.left = position.left - ((selText.length * 5) / 2); */
                        ...
                        /* hack - char difference multiplied by char width in px (font size). */

                        var center = Math.abs((sel.focusOffset - sel.anchorOffset) / 2);

                        console.log('center: ' + center)
                        position = $(".dummy").offset();
                        if (sel.anchorOffset > sel.focusOffset) position.left = position.left - center
                        if (sel.focusOffset > sel.anchorOffset) position.left = position.left + center
                    ...
                    $(settings.selector).css("left", "33%");
                    $(settings.selector).show(200, function() {
